...
class NumberButtonCommand implements ICommand<ResultSet> {
    /*******************************************************************************
     * Variable.
     *******************************************************************************/
    private RequestNumberSet mRequestSet = null;
    ...
    public ResultSet execute(IRequestSet<?> request) {
        mRequestSet = (RequestNumberSet) request;

        // 요청된 command를 실행한다.
        ArrayList<SudokuData> arrItems = mRequestSet.getArraySudokuData();
        arrItems.remove(mRequestSet.getPosition());
        arrItems.add(mRequestSet.getPosition(), new SudokuData(mRequestSet.getRequestID().getRequestIDCode(), true));
